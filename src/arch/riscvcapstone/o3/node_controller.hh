#ifndef O3_NODE_CONTROLLER_H
#define O3_NODE_CONTROLLER_H
...
struct NodeControllerCommand {
    ...
    NodeControllerCommand() : insn(NULL), canWB(false) {}
    NodeControllerCommand(DynInst* insn) : insn(insn), canWB(false) {}
    virtual void setup(NodeController& controller) = 0;
    virtual bool transit(NodeController& controller, PacketPtr pkt) = 0;
    virtual Tick handleAtomic(NodeController& controller) = 0;
...
struct NodeControllerQuery : NodeControllerCommand {
    ...
    NodeControllerQuery(DynInst* insn, NodeID node_id) : 
        NodeControllerCommand(insn),
        nodeId(node_id) {}
    void setup(NodeController& controller) override;
    bool transit(NodeController& controller, PacketPtr pkt) override;
    Tick handleAtomic(NodeController& controller) override;
...
struct NodeControllerRevoke : NodeControllerCommand {
    ...
    NodeControllerRevoke(DynInst* insn, NodeID node_id) : 
        NodeControllerCommand(insn),
        nodeId(node_id) {}
    void setup(NodeController& controller) override;
    bool transit(NodeController& controller, PacketPtr pkt) override;
...
struct NodeControllerRcUpdate : NodeControllerCommand {
    ...
    NodeControllerRcUpdate(DynInst* insn, NodeID node_id, int delta):
        NodeControllerCommand(insn),
...
struct NodeControllerAllocate : NodeControllerCommand {
    ...
    NodeControllerAllocate(DynInst* insn, NodeID parent_id):
        NodeControllerCommand(insn),
        parentId(parent_id) {}
    bool readOnly() const override;
    void setup(NodeController& controller) override;
...
class NodeController : public ClockedObject {
    private:
        ...
        NodeControllerCommandPtr currentCmd; // command currently being handled
    ...
    public:
        ...
        bool executeTiming(NodeControllerCommandPtr cmd);
        Tick executeAtomic(NodeControllerCommandPtr cmd);
